...
; Context Menu
	contextArray :=	["SDK 2018", "Debug", "API", "Getting Started", "Programmer`'s Guide"
				   , "SDK 2018", "Scripting Docs", "Expressions Docs"	
				   , "Key History", "myTmaster", "Toolblox", "Pied Piper", "01 Scripts"						
				   , "All", "Pen Pal", "Pen Palette", "Sourcerer"								
				   , "AI SDK", "AE SDK", "AHK", "CEP", "Adobe", "Visual Studio", "Intro to Scripting", "ExtendScript JS Tools"
	...
	cLabelArray :=	["menu_SDK_AI_Folder", "menu_SDK_AI_Debug", "menu_SDK_AI_API", "menu_SDK_AI_GettingStarted", "menu_SDK_AI_ProgrammersGuide"
			       , "menu_SDK_AE_Folder",  "menu_Script_Docs", "menu_Exp_Docs"
				   ...
				   , ":AISDK", ":AESDK", ":AHK", "menu_CEP_Folder", "menu_Adobe", "menu_VisualStudio", "menu_IntroToScripting", "menu_JavaScriptTools"
...
SDKAInum := 5, SDKAEnum := SDKAInum + 3, AHKnum := SDKAEnum + 5, Illustratornum := AHKnum + 4, Codenum := Illustratornum + 8, Favoritesnum := Codenum + 3, Scriptsnum := Favoritesnum + 3
main_Major := 32
...
; Menu, Scripts, Add
	...
	; submenus
			If (A_Index < main_Major)
				{
					If (A_Index <= SDKAInum) {
						Menu, AISDK, Add, % contextArray[A_Index], % cLabelArray[A_Index]
						Menu, AISDK, Icon, % contextArray[A_Index], %A_WorkingDir%\resources\icons\ContextMenu\%A_Index%.ico
					} Else If (A_Index <= SDKAEnum){
						Menu, AESDK, Add, % contextArray[A_Index], % cLabelArray[A_Index]
						Menu, AESDK, Icon, % contextArray[A_Index], %A_WorkingDir%\resources\icons\ContextMenu\%A_Index%.ico
...
#IfWinActive, ahk_class AE_CApplication_15.1
^Numpad0::Gosub, JumpToExtendScript
#If
...
; Return
	...
	If WinExist("ahk_exe chrome.exe") {
		...
		If WinActive("ahk_exe chrome.exe") {
				SendInput, {Raw}https://github.com/Adobe-CEP/Samples/tree/master/AfterEffectsPanel
				Send, {Enter}
			}
...
#If
		...
		; runScript("Piper 3.2", "C:\Users\PortablePawnShop\Documents\GitMaster\myTmaster")
